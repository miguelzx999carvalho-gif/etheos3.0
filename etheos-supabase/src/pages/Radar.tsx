...
export function Radar() {
  ...
  async function handleSubmit() {
    ...
    await submitRadar(payload)
    setSummary(await getRadarSummary())
    setSubmitted(true)
  }
          ...
          <CardContent className="h-80">
            {hasData && radarData.length > 0 ? (
              ...
              <div className="flex h-full items-center justify-center text-sm text-gray-400">
                {loading ? (
                  <span className="flex items-center gap-2"><Loader2 className="h-5 w-5 animate-spin" /> Carregando...</span>
                ) : hasData ? (
