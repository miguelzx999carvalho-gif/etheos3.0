...
export function Dashboard() {
  ...
  const [radar, setRadar] = useState({ submissions: 0, overall: 0 })
  ...
  useEffect(() => {
    async function load() {
      ...
      const [p, r, d] = await Promise.all([
        getProgress(user.id),
        getRadarSummary(),
        listMyDenuncias(user.id),
      ])
      setProgress(p)
      setRadar({ submissions: r.submissions, overall: r.overall })
      setDenuncias(d)
      setLoading(false)
    }
    load()
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center gap-2 py-16 text-gray-500 dark:text-gray-400">
        <Loader2 className="h-6 w-6 animate-spin" /> Carregando seu painel...
      </div>
    )
  }
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader
            ...
            subtitle="Você só vê as denúncias que registrou"
