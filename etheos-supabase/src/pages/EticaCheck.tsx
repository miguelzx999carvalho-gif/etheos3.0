...
export function EticaCheck() {
  ...
  const [progress, setProgress] = useState({
    ...
    totalPoints: 0,
    maxPoints: scenarios.length * 100,
    percentage: 0,
    scoreLevel: 'Iniciante',
  })
  ...
  useEffect(() => {
    async function load() {
      if (!user) return
      const [ids, prog] = await Promise.all([getAnsweredIds(user.id), getProgress(user.id)])
      setAnsweredIds(ids)
      setProgress(prog)
      setLoading(false)
    }
  ...
  async function handleSelect(index: number) {
    ...
    setSelected(index)
    setRevealed(true)
    await saveAnswer(user.id, current!.id, index)
    setAnsweredIds(await getAnsweredIds(user.id))
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center gap-2 py-16 text-gray-500 dark:text-gray-400">
        <Loader2 className="h-6 w-6 animate-spin" /> Carregando seu progresso...
      </div>
    )
  }
          ...
          {revealed && selectedOption && (
            <div className="mt-6 space-y-4">
              ...
              <div className="flex justify-end">
                ...
                >
                  Próxima situação <ChevronRight className="h-4 w-4" />
                </button>
              </div>
            </div>
          )}
