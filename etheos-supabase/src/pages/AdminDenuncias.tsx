...
import { ShieldCheck, Inbox, RefreshCw, Loader2 } from 'lucide-react'
import { Card, CardHeader, CardContent } from '../components/Card'
import { Badge, statusTone } from '../components/Badge'
import { listAllDenuncias, updateDenunciaStatus } from '../services/denuncias'
...
export function AdminDenuncias() {
  ...
  const refresh = useCallback(async () => {
    setLoading(true)
    setDenuncias(await listAllDenuncias())
    setLoading(false)
  }, [])
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-gray-600 dark:text-gray-300">
            Todas as denúncias registradas por colaboradores aparecem aqui.
          </p>
        </div>
        <button
          onClick={refresh}
            ...
            <CardContent>
              {loading ? (
                <div className="flex items-center justify-center gap-2 py-8 text-sm text-gray-500 dark:text-gray-400">
                  <Loader2 className="h-5 w-5 animate-spin" /> Carregando denúncias...
                </div>
              ) : denuncias.length === 0 ? (
      ...
      <div className="rounded-xl border border-accent-200 bg-accent-50 p-4 text-sm text-accent-800 dark:border-accent-800 dark:bg-accent-900/20 dark:text-accent-200">
        <strong>Segurança:</strong> as denúncias ficam no Supabase com Row-Level Security. Somente
        o papel administrador pode listar e alterar todas. Usuários comuns só enxergam as próprias
        (pelo código).
      </div>
    </div>
  )
