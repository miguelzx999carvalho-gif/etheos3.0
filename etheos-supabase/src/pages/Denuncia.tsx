...
export function Denuncia() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Não foi possível registrar a denúncia.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="grid gap-6 lg:grid-cols-3">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-brand-600 to-accent-600 px-6 py-2.5 text-sm font-semibold text-white shadow-md shadow-accent-500/25 transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
                >
                  <Send className="h-4 w-4" /> {loading ? 'Enviando...' : 'Enviar denúncia'}
                </button>
              </div>
            </form>
          </CardContent>
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-200">
          ...
          <p>
            <strong>Demo:</strong> as denúncias ficam no banco do Supabase. Este não é um canal
